import { ObjectReplaceRecursive } from '../index';

test('It throws an Error when arguments min length are invalid', () => {
  expect(() => ObjectReplaceRecursive({})).toThrowError(
    'There should be at least 2 arguments passed to replaceRecursive()',
  );
});

test('It throws an Error when the first argument have an unexpected property', () => {
  expect(() => ObjectReplaceRecursive(2, {})).toThrowError(
